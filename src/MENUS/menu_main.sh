...
#                                                                                      #
#    Versão:$bright_blue $version     $output_branch
"
echo -e "$debugstatus
########################################################################################
#                                                                                      #
...
#                                                                                      #
#$cyan    (o)$orange                  Visão geral/estado dos programas                                  #
#                                                                                      #
...
########################################################################################
    ...
    Tornei-o muito mais fácil de movimentar. Claro que ainda podes usar a forma antiga,
    mas agora podes saltar para onde queres ir se te lembrares dos comandos

    Por exemplo, para bitcoin, pode escrever:
    $green
                        mb$orange    ou$green   mbitcoin$orange

    para electrs:
                $green
                        mers$orange  ou$green    melectrs$orange

    Para ver todos os atalhos possíveis, dê uma vista de olhos ao código (digite$cyan 'code'$orange) e
    $cyan<enter>$orange a partir daqui... não, na verdade, quase a partir de qualquer lugar - bom, não?
...
$cyan
                ...
                ;;
$orange
    ... isso significa que se você digitar 'mbitcoin' ou 'ub' etc, então o código abaixo, até o ;; será
    executado. Ele diz que se o texto 'bitcoin-end' existir no arquivo ic (abreviação de installed.conf),
    então menu_bitcoin será executado, seguido pela definição de uma flag, que é apenas um sinal para
    uma parte diferente do código saber para onde ir em seguida. A parte 'else' significa sair se 
    bitcoin-end não existir 
...
########################################################################################
    ...
    Pode saltar diretamente para o menu da sua escolha a partir da linha de comando. Por exemplo....$cyan

    rp mb$orange ... para o levar diretamente para o menu Bitcoin.

    ou
$cyan
    rp mbtcp$orange ... para aceder diretamente ao menu BTCPay.
$yellow

    Podem aparecer os habituais ecrãs do pré-menu principal, que podem ser permanentemente
    eliminados da forma habitual, se pretender mais rapidez.
$orange
########################################################################################
"
enter_continue ; jump $enter_cont
