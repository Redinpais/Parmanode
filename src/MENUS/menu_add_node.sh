...
########################################################################################
#                                                                                      #
#    P A R M A N O D E --> Menu principal --> Menu de instalação  -->$cyan Instalar Node $orange               #
#                                                                                      #
########################################################################################
#                                                                                      #
#                                                                                      #
...
########################################################################################
"
choose "xpmq" ; escolha de leitura  
jump $choice || { inválido ; continuar ; } ; set_terminal
case $choice in
q|Q) exit ;; p|P) regressar ;; m|M) back2main ;;
    ...
    lnd|LND|Lnd)
       if [[ -n $lnd_n ]] ; then
         ...
         if [[ -z $lnddocker_n ]] ; then announce "Não é possível fazer isso com o Docker LND. Abortando." ; continue ; fi
...
read
   ...
   litd|LITD) 
       if [[ -n $litd_n ]] ; then
         if [[ -z $lnddocker_n ]] ; then announce "Não é possível fazer isso com o Docker LND. Abortando." ; continue ; fi
         if [[ -z $lnd_n ]] ; then announce "Não pode ser assim com o LND. Abortar." ; continue ; fi
